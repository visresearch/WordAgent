...
本项目是一个基于(多)智能体的AI辅助写作系统：Word Agent，用户在 **办公软件(如WPS、Microsoft Word)** 中安装 **加载项** 后，可以通过自然语言与AI智能体进行交互，获取**写作建议**、**内容生成**、**结构优化**等服务。

对比市面上已有的AI辅助写作工具，文策AI的优势在于：

1. **多版本、跨平台兼容**：以国民级办公软件为载体，让普通用户无门槛获得优质的AI写作辅助体验，并且同时支持Windows和Linux系统。
2. **支持文档样式、段落编辑**：对比常见的在Word中的AI写作工具，本项目智能体能够理解Word文章结构，能够自主联网搜集资料信息，生成符合Word文档结构的内容，能够根据用户需求进行文章结构修改和内容修改。
3. **支持多智能体协作架构**：多智能体扮演不同**专家角色**，以生成有深度的长文章为目标，协同完成写作任务。
4. **支持自定义API或本地服务**：本项目使用的大模型服务APIKey来自于用户自己，目前支持世界上大多数主流的LLM服务商，用户可以根据自己的需求选择不同的LLM服务商和不同的模型。
...
下载后双击exe文件，启动后端服务，打开Word软件，信任加载项，即可体验服务。

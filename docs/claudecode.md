提示词是 Agent 的核心，用于定义大模型如何工作。

你可以如何获取 Prompt
提取方法：Prompt 是最容易被提取出来的，因为纯文本字符串，即使是人眼直接看也是非常容易的。可以简单搜索常见的特定字符串，如 “You are”, “Your task”, “IMPORTANT:”, “Usage:"，就能找到大部分的 Prompt。

以下的 Prompt 是基于 v1.0.50 版本列出，部分有变量或者代码相关部分，我手工替换成更容易理解的代码。

1. 主系统提示（Main System Prompt）
...
- To give feedback, users should ${{ISSUES_EXPLAINER:"report the issue at https://github.com/anthropics/claude-code/issues",PACKAGE_URL:"@anthropic-ai/claude-code",README_URL:"https://docs.anthropic.com/s/claude-code",VERSION:"1.0.50"}.ISSUES_EXPLAINER}
...
# Tool usage policy${AvailableTools.has(Task)?`
- When doing file search, prefer to use the Task tool in order to reduce context usage.`:""}${AvailableTools.has(WebFetch)?`
...
位置：Line 298277-298292 使用场景：内置通用 Agent 的系统提示
...
2.2 Agent 简化版提示
位置：Line 262664-262670 使用场景：简化的 Agent 系统提示
...
位置：Line 282690-282691

Replace the contents of a specific cell in a Jupyter notebook.
Completely replaces the contents of a specific cell in a Jupyter notebook (.ipynb file) with new source.
4. 特殊用途提示
4.1 终端标题生成
位置：Line 270331 使用场景：根据用户输入生成终端窗口标题
...
4.2 文件路径提取
位置：Line 272155-272170 使用场景：从命令输出中提取文件路径

Extract any file paths that this command reads or modifies. For commands like "git diff" and "cat", include the paths of files being shown. Use paths verbatim -- don't add any slashes or try to resolve them. Do not try to infer paths that were not explicitly listed in the command output.

IMPORTANT: Commands that do not display the contents of the files should not return any filepaths. For eg. "ls", pwd", "find". Even more complicated commands that don't display the contents should not be considered...
...
Output: ${B}
4.3 Bash 命令前缀检测
位置：Line 275926-275945 使用场景：检测 Bash 命令的安全性和风险级别
...
5. 任务管理提示
TodoWrite 工具详细说明
位置：Line 261841-262004 使用场景：管理 Agent 的 Todo List
...
6. Task Tool 提示
Task 工具使用说明
位置：Line 298619-298633 使用场景：调用 Agent 执行复杂任务

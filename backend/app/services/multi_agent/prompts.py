...
每个 agent 从 skills/{agent_name}/ 目录加载专用 prompt，
同时合并 skills/common/ 目录的通用规则。
"""
...
def get_agent_prompt(agent_name: str) -> str:
    ...
    """
    parts: list[str] = []
    parts.extend(_load_dir(_SKILLS_DIR / "common"))
    parts.extend(_load_dir(_SKILLS_DIR / f"{agent_name}_agent"))
    if not parts:

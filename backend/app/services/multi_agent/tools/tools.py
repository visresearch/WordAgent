...
def create_workflow(workflow: Workflow) -> str:
    ...
    可用的 agent：
    - research: 负责搜索网络资料（MCP tools 动态加载）
...
class ReviewResult(BaseModel):
    ...
    score: int = Field(description="文档质量综合评分（1-10）", ge=1, le=10)
    feedback: str = Field(description="审核意见，包含优点、不足和给用户的修改建议")
...
def review_document(review: ReviewResult) -> str:
    ...
    你是 Reviewer Agent，负责评估 Writer 生成的文档质量，向用户反馈意见。
    你不会修改文档，只提供评价和建议。
    ...
    feedback 中应包含：文档优点、不足之处、给用户的具体修改建议。

    Args:
        review: 审查结果，包含评分和反馈意见
    ...
    print(f"[review_document] 评分: {review_dict['score']}/10")
...
ALL_TOOLS = [read_document, generate_document, delete_document, search_documnet]
TOOL_MAP = {t.name: t for t in ALL_TOOLS}

# 按 agent 角色分组的工具集
PLANNER_TOOLS = [create_workflow]
# RESEARCH_TOOLS 已在 multi_agent/tools/__init__.py 中定义为 [load_skill_context]
# MCP tools 在运行时动态加载
RESEARCH_TOOLS = []  # 不使用，research agent 通过 __init__.py 中的 RESEARCH_TOOLS + 动态 MCP tools

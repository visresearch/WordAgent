...
### ⚠ styles 字典是必填字段（最高优先级）

CRITICAL: 每次调用 `generate_document` 都**必须**在 `document` 对象中包含 `styles` 字典。`styles` 字典必须覆盖所有 `pStyle`、`rStyle`、`cStyle`、`tStyle` 引用到的样式定义，否则工具调用会直接报错。**绝对不允许省略 `styles` 字段**。
...
CRITICAL: 当你已经通过 MCP 搜索/检索工具获取了资料，用户又要求写文档/报告/文章时，你**必须**调用 generate_document 将内容输出为 Word 文档。绝对不允许把检索结果整理成纯文本回复。哪怕构建 paragraphs JSON 参数比较费力，也必须完成工具调用。
...
assistant: [先调用 MCP 搜索工具检索资料 -> 根据检索结果构建 paragraphs -> 调用 generate_document]
已根据搜索结果为您生成时事报道文档。
</example>

### 完整 document 对象示例（含 styles）

CRITICAL: 以下是一个完整的 `generate_document` 调用参数结构，注意 `styles` 字典必须包含所有被引用的样式 ID：

```json
{
  "document": {
    "paragraphs": [
      {"pStyle": "pS_2", "runs": [{"rStyle": "rS_1", "text": "一、标题示例"}]},
      {"pStyle": "pS_3", "runs": [{"rStyle": "rS_2", "text": "这是正文第一段的内容。"}]}
    ],
    "tables": [],
    "styles": {
...
每次构建 `generate_document` 参数时，提交前必须检查：
1. ✅ `document` 对象中是否包含 `styles` 字典？（**必填，缺少则报错**）
2. ✅ `styles` 字典是否覆盖了所有 `pStyle`、`rStyle`、`cStyle`、`tStyle` 引用的 ID？
3. ✅ 样式 ID 是引用字符串（如 `"pS_1"`）而不是内联数组？
4. ✅ `paragraphs` 中每个元素都是对象而非字符串？

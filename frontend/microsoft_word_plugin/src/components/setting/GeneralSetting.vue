  ...
  <div class="general-setting-container">
    ...
    <div class="section-header">
      ...
      <div class="section-title-group">
        ...
        <p class="section-subtitle">配置应用的启动行为和显示模式</p>
      </div>
    </div>
    ...
    <div class="setting-group">
      <div class="group-title">校对显示模式</div>
      <div class="radio-group">
        <label class="radio-item" :class="{ active: localSettings.proofreadMode === 'redblue' }">
          <input v-model="localSettings.proofreadMode" type="radio" value="redblue" @change="emitChange" />
        ...
        <label class="radio-item" :class="{ active: localSettings.proofreadMode === 'revision' }">
          <input v-model="localSettings.proofreadMode" type="radio" value="revision" @change="emitChange" />
          <span class="radio-circle"></span>
          <div class="radio-content">
    ...
    <div class="section-header">
      ...
      <div class="section-title-group">
        ...
        <p class="section-subtitle">配置 HTTP/HTTPS 请求的代理服务器</p>
      </div>
    </div>
      ...
      <div class="proxy-inputs" :class="{ disabled: !localSettings.proxy.enabled }">
        <div class="proxy-row">
          <div class="input-group flex-grow">
            ...
            <input v-model="localSettings.proxy.host" type="text" class="text-input" placeholder="127.0.0.1" :disabled="!localSettings.proxy.enabled" @input="emitChange" />
          </div>
          <div class="input-group port-input">
            <label class="input-label">端口</label>
            <input v-model.number="localSettings.proxy.port" type="number" class="text-input" placeholder="7897" min="1" max="65535" :disabled="!localSettings.proxy.enabled" @input="emitChange" />
          </div>
        </div>
        <p class="proxy-hint">提示：HTTP 和 HTTPS 请求将统一使用此代理。仅支持 HTTP 代理协议，不支持 SOCKS。</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    return { localSettings, emitChange };
  }
};
</script>

<style scoped>
.general-setting-container { padding: 0; }
...
.section-title { font-size: 18px; font-weight: 600; margin: 0 0 4px 0; color: #2c3e50; }
...
.group-title { font-size: 14px; color: #333; margin-bottom: 16px; font-weight: 500; }

.radio-group { display: flex; flex-direction: column; gap: 12px; }
.radio-item {
  display: flex; align-items: flex-start; gap: 12px; padding: 16px;
  background: white; border: 1px solid #e8e8e8; border-radius: 8px; cursor: pointer; transition: all 0.2s;
}
.radio-item:hover { border-color: #d0d0d0; }
.radio-item.active { border-color: #667eea; background: #f8faff; }
.radio-item input { display: none; }
.radio-circle {
  width: 18px; height: 18px; border: 2px solid #d0d0d0; border-radius: 50%;
  flex-shrink: 0; margin-top: 2px; position: relative; transition: all 0.2s;
}
.radio-item.active .radio-circle { border-color: #667eea; }
...
.radio-title { font-size: 14px; color: #333; font-weight: 500; }
.radio-desc { font-size: 12px; color: #888; }

.proxy-section { background: white; border: 1px solid #e8e8e8; border-radius: 8px; padding: 12px 16px; }
.proxy-inputs { display: flex; flex-direction: column; gap: 12px; transition: opacity 0.2s; }
.proxy-inputs.disabled { opacity: 0.5; pointer-events: none; }
.proxy-row { display: flex; gap: 12px; align-items: flex-end; }
.flex-grow { flex: 1; }
.port-input { width: 90px; flex-shrink: 0; }
.input-group { display: flex; flex-direction: column; gap: 4px; }
.input-label { font-size: 13px; color: #555; font-weight: 500; }
.text-input {
...
.text-input:focus { border-color: #667eea; background: white; }
.text-input:disabled { background: #f5f5f5; color: #aaa; cursor: not-allowed; }
...
.proxy-hint { font-size: 12px; color: #999; margin: 4px 0 0 0; line-height: 1.5; }
.section-divider { height: 1px; background: #e8e8e8; margin: 32px 0; }
</style>

 ...
 *
 * 使用方式：
 * import { executeQuery, generateSummary } from './docxQuery.js'
 *
...
export function executeQuery(docJson, queryDSL) {
  ...
  // 提取匹配的段落（带元信息）
  const matchedParagraphs = extractParagraphs(
    paragraphs,
    expandedIndices,
    limitedIndices,
    query,
...
function executeBoolQuery(paragraphs, bool) {
  ...
  // 处理 should - 至少匹配 N 个
  if (should.length > 0) {
    const shouldMatches = should.map((clause) => new Set(executeQueryClause(paragraphs, clause)));

...
除此之外，本项目还支持用户自定义工具的接入，用户可以通过配置MCP服务器的方式让智能体调用第三方API来增强智能体的能力。以**Tavily智搜**和**可视化图表-MCP-Server**为例，用户输入“请你查询今天长沙气温，做一个最近一周的气温表格，第一排是日期，第二排是气温”。智能体会调用Tavily智搜MCP服务器进行联网搜索获取长沙最近一周的气温数据,然后调用generate_document工具生成一个包含表格的Word文档内容返回给前端加载项，前端加载项根据智能体返回的内容将表格渲染出来。然后用户将表格引用再提问“请你根据这个表格，画一个气温柱状图”，智能体会调用read_document工具理解文档中的表格，然后调用可视化图表-MCP-Server这个MCP服务器生成一个柱状图的图片URL，把这张图片渲染在前端加载项界面中。

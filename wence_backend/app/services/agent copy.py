...
class Run(BaseModel):
    ...
    fontName: str = Field("宋体", description="字体名称，如 '宋体', 'Times New Roman'")
    fontSize: float = Field(12, description="字号")
    bold: bool = Field(False, description="是否加粗")
    italic: bool = Field(False, description="是否斜体")
    underline: str = Field("none", description="下划线：none/single/double/thick")
    color: str = Field("#000000", description="颜色，如 '#000000'")
    highlight: str = Field("none", description="高亮色")
    strikethrough: bool = Field(False, description="是否删除线")
...
class Paragraph(BaseModel):
    ...
    alignment: str = Field("left", description="对齐方式：left/center/right/justify")
    lineSpacing: float = Field(12, description="行间距")
    indentLeft: float = Field(0, description="左缩进（磅）")
    indentRight: float = Field(0, description="右缩进（磅）")
    indentFirstLine: float = Field(0, description="首行缩进（磅）")
    spaceBefore: float = Field(0, description="段前间距（磅）")
    spaceAfter: float = Field(6, description="段后间距（磅）")
    styleName: str = Field("正文", description="样式名称，如 '标题 1', '标题 2', '正文'")
...
class Cell(BaseModel):
    ...
    rowSpan: int = Field(1, description="跨行数")
    colSpan: int = Field(1, description="跨列数")
    alignment: str = Field("left", description="水平对齐")
    verticalAlignment: str = Field("top", description="垂直对齐")
...
class Table(BaseModel):
    ...
    tableAlignment: str = Field("left", description="表格对齐：left/center/right")
...
class DocumentOutput(BaseModel):
    ...
    tables: list[Table] = Field(default_factory=list, description="表格数组（如果没有表格则为空列表）")

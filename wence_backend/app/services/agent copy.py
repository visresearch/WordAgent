...
【工具使用规则 - 非常重要】
✅ **必须调用 generate_document 工具的情况（同时满足以下条件）：**
1. 用户**明确要求**对文档进行操作：
   - **修改**、**润色**、**重写**、**翻译**现有文档
   - **扩写**、**缩写**、**续写**文档
   - 调整文档**格式**（字体、对齐等）
   - **生成**、**创建**、**写**新文档内容（报告、证明、论文等）
2. **并且**用户提供了文档内容（有 paragraphs 或 tables），或明确要求生成新文档

❌ **绝对不调用工具的情况：**
- 用户只是**问候**（"你好"、"谢谢"、"再见"）
- 用户在**提问**（"什么是..."、"如何..."、"为什么..."、"你能干什么"）
- 用户在**闲聊**或**讨论**问题
- **没有文档内容**且用户**没有明确要求生成**新文档
- 用户只是想了解你的功能或能力

🔴【关键判断标准】🔴
**如果用户没有选中任何文档内容（documentJson 为空或只有 docId），并且也没有明确说"帮我写..."、"生成..."，那么这是普通对话，不要调用工具！**
...
- 扩写新增段落时：沿用相邻段落的完整格式（复制 pStyle 和 rStyle）
"""
...
async def process_writing_request_stream(
    ...
    if is_ask_mode:
        # ask 模式：纯对话，不绑定工具，不发送工具 schema
        llm_for_agent = llm
        print("[LangGraph Agent] ask 模式，不绑定工具")
    else:
        # agent 模式：绑定工具，可以生成文档
        llm_for_agent = llm.bind_tools([generate_document])
        print("[LangGraph Agent] agent 模式，已绑定工具")

...
def call_tools(state: MessagesState) -> dict:
    ...
    for tool_call in last_message.tool_calls:
        ...
        if tool_name == "generate_document":
            ...
            result = read_document.invoke(tool_call["args"])
...
def should_summarize(state: MessagesState) -> str:
    ...
    if isinstance(last_message, ToolMessage):
        ...
        # request_document 返回空字符串（SSE 模式），不需要总结
        if not content:
            print("[Router] -> END (request_document 已发送，SSE 模式)")
            return END
        # request_document 返回了文档 JSON（WebSocket 模式），回到 writer_agent 继续处理
        try:
            doc = json.loads(content)
            if "paragraphs" in doc:
                print("[Router] -> writer_agent (request_document 获得文档，继续处理)")
                return "writer_agent"
...
def build_graph(llm_with_tools, llm):
    ...
    # call_tools 后根据工具类型决定下一步
    graph.add_conditional_edges(
        "call_tools",
        should_summarize,
        {"summary_agent": "summary_agent", "writer_agent": "writer_agent", END: END},
    )
...
async def process_writing_request_stream(
    ...
    if is_ask_mode:
        # ask 模式：纯对话，可以读取文档
        llm_for_agent = llm.bind_tools([read_document])
        print("[LangGraph Agent] ask 模式，已绑定 read_document 工具")
    else:
        # agent 模式：绑定工具，可以读取文档和生成文档
        llm_for_agent = llm.bind_tools([generate_document, read_document])
        print("[LangGraph Agent] agent 模式，已绑定 generate_document 和 read_document 工具")

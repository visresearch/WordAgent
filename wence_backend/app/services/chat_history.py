...
from app.models.db_models import ChatMessage, Document
...
class ChatHistoryService:
    ...
    async def get_or_create_document(self, doc_id: str, doc_name: str | None = None) -> Document:
        """
        获取或创建文档记录
    ...
    async def get_history(self, doc_id: str, limit: int = 50, offset: int = 0) -> list[dict]:
        """
        获取指定文档的聊天历史
        ...
        """
        # 先查找文档
        doc_result = await self.db.execute(select(Document).where(Document.doc_id == doc_id))
    ...
    async def clear_history(self, doc_id: str) -> bool:
        ...
        await self.db.execute(delete(ChatMessage).where(ChatMessage.document_id == document.id))
        await self.db.flush()
        return True
    ...
    async def get_all_documents(self, limit: int = 100) -> list[dict]:
        ...
        Returns:
            文档列表
        """
        result = await self.db.execute(select(Document).order_by(desc(Document.updated_at)).limit(limit))
